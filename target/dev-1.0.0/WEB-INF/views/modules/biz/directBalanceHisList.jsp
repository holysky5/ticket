...
<body>
	<ul class="nav nav-tabs">
		...
		<li class="active"><a href="${ctx}/biz/directBalanceHis/list?direct.id=${directBalanceHis.direct.id}">账户明细</a></li>
	...
	<form:form id="searchForm" modelAttribute="directBalanceHis" action="${ctx}/biz/directBalanceHis/list?direct.id=${directBalanceHis.direct.id}" method="post" class="breadcrumb  form-search">
		<input id="pageNo" name="pageNo" type="hidden" value="${page.pageNo}"/>
		<input id="pageSize" name="pageSize" type="hidden" value="${page.pageSize}"/>
		<span class="label label-success">${direct.distributor.name}-景区预存款可使用余额：<fmt:formatNumber type="currency" pattern="#,##0.00#">${direct.balance}</fmt:formatNumber>元</span>&nbsp;&nbsp;
