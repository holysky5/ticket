...
<body>
		...
		<table class="table table-striped table-condensed " style="width:600px;margin-top:20px;" >
			...
			<tbody>
		   	...
		   	<tr>
				...
				<td><span class="uneditable-input" style="color:red;"><fmt:formatNumber type="currency" pattern="#,##0.00#"> ${product.platformPrice }</fmt:formatNumber>元</span></td>
			</tr>
		   	<tr>
		   	...
		   	<tr>
				...
				<td><span class="uneditable-input" style="color:red;" id="pre_count_price">0.0元</span></td>
		   	</tr>
		   		<tr>
	...
	<ul class="nav nav-tabs">
		...
		<li><a href="${ctx}/biz/product/listProduct4Distributor/${supplier.id}">${product.scenic.name}</a></li>
		<li class="active"><a href="${ctx}/biz/orderInfo/prepareOrder?productNo=${product.no}">${product.scenic.name} - ${product.name} - 下单</a></li>
	</ul><br/>
	<form:form id="inputForm" modelAttribute="orderInfo" action="${ctx}/biz/orderInfo/createOrder" method="post"  class="form-horizontal">
		...
		<input type="hidden" id="purchasePrice" name="purchasePrice" value="${product.platformPrice }"/>
		...
		<table class="table table-striped table-bordered table-condensed " >
			...
			<tr>
		   	...
		   	<tr>
				...
				<td>
				<form:input path="useDate" autocomplete="off" htmlEscape="false" maxlength="20" readonly="readonly" class="Wdate required"  onclick="WdatePicker({startDate:'%y-%M-%d ',dateFmt:'yyyy-MM-dd',isShowClear:false,minDate:'${minday}',maxDate:'${maxday}'});"/>
				<span class="label label-success">${product.effectTimeDesc}</span>
				
				<c:if test="${not empty notbegin}">
				<span class="label label-success">${product.beignAndEndTimeDesc}</span>
				</c:if>
				</td>
				<td style="width:80px;"><span class="asterisk">*</span><b>结算单价:</b></td>
				<td><span class="uneditable-input" style="color:red;"> <fmt:formatNumber type="currency" pattern="#,##0.00#">${product.platformPrice }</fmt:formatNumber>元</span><span class="label label-success">挂牌价：<fmt:formatNumber type="currency" pattern="#,##0.00#">${product.originalPrice }</fmt:formatNumber>元</span></td>
			</tr>
		   	<tr>
				...
				<td><span id="count_price"  class="uneditable-input" style="color:red;">0.00元</span><span class="label label-success">账户余额：<fmt:formatNumber type="currency" pattern="#,##0.00#">${distributor.balance}</fmt:formatNumber>元</span></td>
		   	</tr>
		   	<tr>
		...
		<div class="form-actions">
			...
			<c:if test="${not empty notbegin}">
			<input  class="btn btn-primary disabled" type="button" value="暂时不能预订"/>
			</c:if>
			<input id="btnCancel" class="btn" type="button" value="返 回" onclick="history.go(-1)"/>
			<c:if test="${product.scenic.checkTerminal == 9 }">
			<span style="margin-left:20px;margin-right:20px;color:red;">该景区门票为后台人工处理，提交或修改订单工作人员当日完成</span>
			</c:if>
		</div>
	</form:form>
</body>
