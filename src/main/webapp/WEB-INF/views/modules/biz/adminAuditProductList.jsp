...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/biz/product/listWait4AuditProduct">待审核门票列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="product" action="${ctx}/biz/product/listWait4AuditProduct" method="post" class="breadcrumb form-search">
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed">
		<thead><tr><th style="width:20px;">#</th><th>供应商</th><th>门票编号</th><th>门票名称</th><th>挂牌价</th><th>指导价</th><th>采购价</th><th>状态</th><th>审核</th><th>发布时间</th><th>更新时间</th><shiro:hasPermission name="biz:product:edit"><th>操作</th></shiro:hasPermission></tr></thead>
		<tbody>
			...
			<tr>
				...
				<td>
					...
					<c:if test="${product.auditFlag==1}">
						<img src="${ctxStatic}/images/true.png" title="审核通过">
					</c:if>
					<c:if test="${product.auditFlag==0}">
						<c:if test="${product.updateBy.userType == 1}">
						<img src="${ctxStatic}/images/false.png" title="审核未通过">
						</c:if>
				...
				<shiro:hasPermission name="biz:product:edit"><td>
					<a href="${ctx}/biz/product/auditForm?no=${product.no}" class="btn btn-mini btn-primary"><i class="icon-flag icon-white"></i> 审核</a>
