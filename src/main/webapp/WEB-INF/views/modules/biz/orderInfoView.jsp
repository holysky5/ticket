	...
	<script type="text/javascript">
		$(document).ready(function() {
			$("#btnSms").click(function(){
				$.ajax({
					url:"${ctx}/biz/orderInfo/sendSms",
					data:{orderid: '${orderInfo.orderId}'},
					success:function(data){
						if(data == 'true'){
							top.jBox.info('发送成功', '提示');
						}else{
							top.jBox.info('发送失败', '提示');
						}
					}
				});
			})
...
<body>
	...
	<form:form id="inputForm" modelAttribute="orderInfo" action="${ctx}/biz/orderInfo/createOrder" method="post"  class="form-horizontal">
		<form:hidden path="id"/>
		<input type="hidden" id="proHistoryId" name="proHistoryId" value="${product.id}"/>
		...
		<table class="table table-striped table-bordered table-condensed " >
			...
			<c:if test="${(orderInfo.type == '1' &&  user.userType =='4') || user.userType !='4' }" >
		   	<tr>
			...
			<c:if test="${orderInfo.status == '0' && product.scenic.checkTerminal != 9}">
			<tr>
				...
				<td><span class="uneditable-input" style="color:red;">${orderInfo.uuOrdernum}</span>
				<c:if test="${orderInfo.uuRremsg < 3}">
				<input id="btnSms" class="btn btn-mini btn-primary" type="button" value="信息重发" />
				</c:if>
				</td>
