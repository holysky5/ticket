...
<body>
	<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/biz/supplier/list/product?stopTime=${stopTime}">景区列表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="supplier" action="${ctx}/biz/supplier/list/product?stopTime=${stopTime}" method="post" class="breadcrumb form-search">
		...
		<c:forEach items="${page.list}" var="supplier" varStatus="status">
			<tr>
				...
				<td>
					...
					<a href="${ctx}/biz/product/listProduct4Admin/${supplier.id}?stopTime=${stopTime}" class="btn btn-mini btn-primary"><i class="icon-th-list icon-white"></i> 门票</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
