...
public class Product extends DataEntity {
	...
	/** 有效日期. */
	private String startTime = "2014-01-01";

	/** 有效日期. */
	private String stopTime = "2099-01-01";
	...
	@Transient
	public String getStartAndStopTimeDesc() {
		if (StringUtils.isNotBlank(startTime)
				|| StringUtils.isNotBlank(stopTime)) {
			return (startTime == null ? "2014-01-01" : startTime) + "至"
					+ (stopTime == null ? "2099-09-09" : stopTime);
		}
		return "长期有效";
	}

	@Transient
	public boolean getStartAndStopTimeStatus() {
		if (StringUtils.isNotBlank(startTime)) {
			if (DateUtils.getDate().compareTo(startTime) < 0) {
				// this.setStatus("1");
				return false;
			}
		}
		if (StringUtils.isNotBlank(stopTime)) {
			if (DateUtils.getDate().compareTo(stopTime) > 0) {
				// this.setStatus("1");
				return false;
			}
		}
